...
// raw_vec_shim.rs for why a byte-faithful RawVec copy does not compile on this toolchain).
// The deque body below uses it exactly as upstream uses RawVec.
...
pub use self::drain::VVecDequeDrain;

#[path = "drain.rs"] mod drain;

pub use self::extract_if::VVecDequeExtractIf;

#[path = "extract_if.rs"] mod extract_if;

pub use self::iter_mut::VVecDequeIterMut;

#[path = "iter_mut.rs"] mod iter_mut;

pub use self::into_iter::VVecDequeIntoIter;

#[path = "into_iter.rs"] mod into_iter;

pub use self::iter::VVecDequeIter;

#[path = "iter.rs"] mod iter;
...
pub use self::splice::VVecDequeSplice;
...
/// push onto the back in this manner, and iterating over `VVecDeque` goes front
/// to back.
///
/// A `VVecDeque` with a known list of items can be initialized from an array:
///
///
/// Since `VVecDeque` is a ring buffer, its elements are not necessarily contiguous
/// in memory. If you want to access the elements as a single slice, such as for
/// efficient sorting, you can use [`make_contiguous`]. It rotates the `VVecDeque`
/// so that its elements do not wrap, and returns a mutable slice to the
/// now-contiguous element sequence.
///
/// [`push_back`]: VVecDeque::push_back
/// [`pop_front`]: VVecDeque::pop_front
/// [`extend`]: VVecDeque::extend
/// [`append`]: VVecDeque::append
/// [`make_contiguous`]: VVecDeque::make_contiguous
pub struct VVecDeque<
    T,
     A: Allocator = Global,
> {
    ...
    len: usize,
    buf: VRawVec<T, A>,
    // Forget/leak resistance for `drain` and `extract_if` (the lazy_loss_recovery difference from `std`).
    //
    // While a `drain()`/`extract_if()` iterator is live, the deque is "parked" (its `len` is short),
    // exactly as the std variant does. `std` relies solely on the iterator's `drop` to finish the
    // removal and restore `len`; a `mem::forget` skips that `drop` and loses elements. Here the
    // constructor ALSO records the finish work in `pending`, and the iterator mirrors its progress
    // into it on every step. If the iterator's `drop` runs (normally or on a panic unwind) it
    // finishes and clears `pending`. If it is forgotten, `pending` survives, and the next operation
    // on the deque (`restore_wf_wo_data_loss`, run at every entry point and in the deque's own
    // `Drop`) finishes the removal — so nothing leaks. `len()` reports the post-removal length while
    // parked without mutating, so the deque looks finished from the outside. The two cases are
    // mutually exclusive (you cannot hold both iterators at once), so a single slot suffices.
    // BOXED (8 B pointer, not 40 B inline) — the always-present field must be tiny so it does
    // not tax cheap ops like `pop_front`/`new`; allocates only on the rare forget-prone ops.
    pending: Option<Box<Pending>>,
}

// The work `restore_wf_wo_data_loss` must do to finish a forgotten iterator — the same finish the
// iterator's own `drop` would have done. One variant per forget-prone iterator.
enum Pending {
    // `drain`: `idx`/`remaining` track how far iteration got; `drain_len`/`tail_len` size the
    // drained range and the elements after it. (Parked `len` == head length.)
    Drain { drain_len: usize, tail_len: usize, idx: usize, remaining: usize },
...
impl<T: Clone, A: Allocator + Clone> Clone for VVecDeque<T, A> {
    fn clone(&self) -> Self {
...
unsafe impl<#[may_dangle] T, A: Allocator> Drop for VVecDeque<T, A> {
    fn drop(&mut self) {
        // lazy_loss_recovery: if a `drain()` iterator was forgotten, finish it first so the un-yielded drained
        // elements are dropped (not leaked) and `len` covers the full survivor set before we drop it.
...
impl<T> Default for VVecDeque<T> {
    /// Creates an empty deque.
    #[inline]
    fn default() -> VVecDeque<T> {
        VVecDeque::new()
    }
}

impl<T, A: Allocator> VVecDeque<T, A> {
    ...
    /// Assumes capacity is sufficient.
    /// Equivalent to calling [`VVecDeque::copy_slice`] with a [reversed](https://doc.rust-lang.org/std/primitive.slice.html#method.reverse) slice.
    #[inline]
    ...
    ///
    /// If the returned `VVecDequeExtractIf` is not exhausted, e.g. because it is dropped without iterating
    /// or the iteration short-circuits, then the remaining elements will be retained.
    /// Use `extract_if().for_each(drop)` if you do not need the returned iterator,
    /// or [`retain_mut`] with a negated predicate if you also do not need to restrict the range.
    ///
    /// [`retain_mut`]: VVecDeque::retain_mut
    ///
    ...
    ///
    pub fn extract_if<F, R>(&mut self, range: R, filter: F) -> VVecDequeExtractIf<'_, T, F, A>
    where
        F: FnMut(&mut T) -> bool,
        R: RangeBounds<usize>,
    {
        self.restore_wf_wo_data_loss(); // lazy_loss_recovery: finish a forgotten drain/extract_if first
        VVecDequeExtractIf::new(self, filter, range)
    }
}
...
impl<T> VVecDeque<T> {
    ...
    #[inline]
    #[must_use]
    pub const fn new() -> VVecDeque<T> {
        // FIXME(const-hack): This should just be `VVecDeque::new_in(Global)` once that hits stable.
        VVecDeque { head: 0, len: 0, buf: VRawVec::new(), pending: None }
    }
    ...
    #[inline]
    #[must_use]
    pub fn with_capacity(capacity: usize) -> VVecDeque<T> {
    ...
    ///
    #[inline]
    pub fn try_with_capacity(capacity: usize) -> Result<VVecDeque<T>, TryReserveError> {
        Ok(VVecDeque {
            head: 0,
            len: 0,
            buf: VRawVec::try_with_capacity_in(capacity, Global)?,
            pending: None,
        })
    }
}
...
impl<T, A: Allocator> VVecDeque<T, A> {
    ...
    ///
    #[inline]
    pub const fn new_in(alloc: A) -> VVecDeque<T, A> {
        VVecDeque { head: 0, len: 0, buf: VRawVec::new_in(alloc), pending: None }
    }
    ...
    /// # Examples
    ///
    pub fn with_capacity_in(capacity: usize, alloc: A) -> VVecDeque<T, A> {
        VVecDeque { head: 0, len: 0, buf: VRawVec::with_capacity_in(capacity, alloc), pending: None }
    }

    /// Creates a `VVecDeque` from a raw allocation, when the initialized
    ...
    #[inline]
    // Faithful pub(crate) helper: in `alloc`, other modules (slice/boxed conversions) call
    // this; no caller exists within the extracted `vec_deque` subset, so it reads as dead
    // code here. Kept at its original visibility rather than dropped (rule 5.8 justified allow).
    #[allow(dead_code)]
    pub(crate) unsafe fn from_contiguous_raw_parts_in(
        ...
        // and that the allocation is valid for use in `VRawVec`.
        unsafe {
            VVecDeque {
                head: initialized.start,
                len: initialized.end.unchecked_sub(initialized.start),
                buf: VRawVec::from_raw_parts_in(ptr, capacity, alloc),
                pending: None,
            }
        }
    }
    ...
    ///
    /// [`try_reserve`]: VVecDeque::try_reserve
    ///
    /// # Errors
    ///
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        // * The head of the VVecDeque is moved before calling `drop_in_place`,
    ...
    /// # Examples
    ///
    pub fn iter(&self) -> VVecDequeIter<'_, T> {
        let (a, b) = self.as_slices();
        VVecDequeIter::new(a.iter(), b.iter())
    }
    ...
    /// # Examples
    ///
    pub fn iter_mut(&mut self) -> VVecDequeIterMut<'_, T> {
        let (a, b) = self.as_mut_slices();
        VVecDequeIterMut::new(a.iter_mut(), b.iter_mut())
    }
    ...
    pub fn len(&self) -> usize {
        // lazy_loss_recovery: while a forgotten iterator is pending, `self.len` is parked short. Report the
        // post-removal length without mutating, so the deque looks finished from the outside even
        // before the next op runs `restore_wf_wo_data_loss`.
        match self.pending.as_deref() {
            // drain parks `len` at the head length; post-drain length is head + tail.
            Some(Pending::Drain { tail_len, .. }) => self.len + tail_len,
    ...
    /// a `VVecDequeDrain`, then `self.len` is not actually the length of the deque.
    ///
    /// # Safety
    ///
    ...
    ///
    #[inline]
    pub fn range<R>(&self, range: R) -> VVecDequeIter<'_, T>
    where
        R: RangeBounds<usize>,
    {
    ...
    ///
    #[inline]
    pub fn range_mut<R>(&mut self, range: R) -> VVecDequeIterMut<'_, T>
    where
        R: RangeBounds<usize>,
    {
        let (a_range, b_range) = self.slice_ranges(range, self.len);
    ...
    ///
    #[inline]
    pub fn drain<R>(&mut self, range: R) -> VVecDequeDrain<'_, T, A>
    where
        R: RangeBounds<usize>,
    {
        // Memory safety
        //
        // When the VVecDequeDrain is first created, the source deque is shortened to
        // make sure no uninitialized or moved-from elements are accessible at
        // all if the VVecDequeDrain's destructor never gets to run.
        //
        // VVecDequeDrain will ptr::read out the values to remove.
        // When finished, the remaining data will be copied back to cover the hole,
        // and the head/tail values will be restored correctly.
        //
        // lazy_loss_recovery: pass `track_for_forget_safety = true` so a forgotten iterator is finished
        // by the next deque op (`restore_wf_wo_data_loss`). The parking + the comment below are unchanged from std.
        //
        ...
        //
        // We store drain_start as self.len, truncating the effective array; if the iterator's `drop`
        // does not run, `restore_wf_wo_data_loss` (driven by `pending`) finishes the removal so nothing leaks.
        self.drain_core(range, true)
    }

    // Shared core for `drain` (track_for_forget_safety = true: record forget-safety in `pending`) and
    // `splice` (track_for_forget_safety = false: the held drain always drops, so std behavior, no record).
    fn drain_core<R>(&mut self, range: R, track_for_forget_safety: bool) -> VVecDequeDrain<'_, T, A>
    where
        R: RangeBounds<usize>,
    {
        // Finish any drain whose iterator was forgotten, before starting a new one.
        self.restore_wf_wo_data_loss();
        let Range { start, end } = slice::range(range, ..self.len);
        // SAFETY: `slice::range` guarantees `start <= end <= self.len`.
        unsafe { VVecDequeDrain::new(self, start, end - start, track_for_forget_safety) }
    }

    // lazy_loss_recovery forget-safety. Called by the iterator constructors (track_for_forget_safety path) to record the
    // finish work; the iterator mirrors its progress into it via `mirror_progress`.
    pub(super) fn set_pending_drain(&mut self, drain_len: usize, tail_len: usize, idx: usize, remaining: usize) {
        self.pending = Some(Box::new(Pending::Drain { drain_len, tail_len, idx, remaining }));
    }
    ...
    // lazy_loss_recovery forget-safety guard. If a `drain()`/`extract_if()` iterator was forgotten, its finish
    // work is still recorded in `pending`; run it now (finish the removal, restore `len`) so the
    // deque is in its expected post-removal state before this op proceeds. No-op when nothing is
    // pending — a single predictable branch on the hot path.
    #[inline]
    fn restore_wf_wo_data_loss(&mut self) {
        // Common path is a pure read (nothing pending) — avoid `Option::take`, which would store
        // `None` back on every call and dirty the cache line on the hot path.
    ...
    fn restore_wf_wo_data_loss_cold(&mut self) {
        // SAFETY: `pending` was set by `drain()`/`extract_if()` with `len` parked and the buffer
        // otherwise intact; each variant describes exactly the un-finished removal.
        match *self.pending.take().unwrap() {
            Pending::Drain { drain_len, tail_len, idx, remaining } => unsafe {
                VVecDequeDrain::finish_forgotten_drain(self, drain_len, idx, tail_len, remaining)
            },
    ...
    ///
    /// `range` is removed even if the `VVecDequeSplice` iterator is not consumed before it is dropped.
    ///
    /// lazy_loss_recovery forget-safety (differs from std): if the `VVecDequeSplice` value is **leaked**
    /// (`mem::forget`), the deque's own elements are **not** lost — the next deque operation lazily
    /// repairs the deque to well-formed (the `range` is removed and its elements dropped, the tail is
    /// kept). The one thing that does leak is the `replace_with` iterator itself: it is only consumed
    /// when the `VVecDequeSplice` is dropped, and a leaked Splice never drops it, so its un-inserted
    /// items leak. That is unavoidable here without an `I: 'static` bound (a borrowing `replace_with`
    /// cannot be stashed on the deque past the splice's borrow), and `splice`'s signature is kept
    /// identical to std. So: the deque is repaired; only the abandoned replacement input leaks.
    ///
    /// This is optimal if:
    ///
    ...
    /// Panics if the range has `start_bound > end_bound`, or, if the range is
    /// bounded on either end and past the length of the deque.
    pub fn splice<R, I>(&mut self, range: R, replace_with: I) -> VVecDequeSplice<'_, I::IntoIter, A>
    where
        R: RangeBounds<usize>,
        I: IntoIterator<Item = T>,
    {
        VVecDequeSplice { drain: self.drain_core(range, true), replace_with: replace_with.into_iter() }
    }
    ...
    ///
    /// Note that if you have a sorted `VVecDeque`, [`binary_search`] may be faster.
    ///
    /// [`binary_search`]: VVecDeque::binary_search
    ///
    /// # Examples
    ///
    ...
    /// # Examples
    ///
    #[must_use = "if you don't need a reference to the value, use `VVecDeque::push_front` instead"]
    ...
    /// # Examples
    ///
    #[must_use = "if you don't need a reference to the value, use `VVecDeque::push_back` instead"]
    ...
    ///
    /// To get behavior like [`append`][VVecDeque::append] where elements are moved
    /// from the other collection to this one, use `self.prepend(other.drain(..))`.
    ///
    /// # Examples
    ///
    ///
    /// Move values between collections like [`append`][VVecDeque::append] does but prepend to the front:
    ///
    #[track_caller]
    ...
    /// Prepends all contents of the iterator to the front of the deque,
    /// as if [`push_front`][VVecDeque::push_front] was called repeatedly with
    /// the values yielded by the iterator.
    ///
    /// # Examples
    ///
    ///
    /// This behaves like [`push_front`][VVecDeque::push_front] was called repeatedly:
    ///
    #[track_caller]
    ...
    /// # Examples
    ///
    #[must_use = "if you don't need a reference to the value, use `VVecDeque::insert` instead"]
    ...
    ///
    /// Returns a newly allocated `VVecDeque`. `self` contains elements `[0, at)`,
        ...
        A: Clone,
    {
        ...
        let mut other = VVecDeque::with_capacity_in(other_len, self.allocator().clone());
    ...
    ///
    /// [`as_slices`]: VVecDeque::as_slices
    /// [`as_mut_slices`]: VVecDeque::as_mut_slices
    ///
    /// # Examples
    ///
    ...
    /// Binary searches this `VVecDeque` for a given element.
    /// If the `VVecDeque` is not sorted, the returned result is unspecified and
    /// meaningless.
    ///
    ...
    ///
    /// [`binary_search_by`]: VVecDeque::binary_search_by
    /// [`binary_search_by_key`]: VVecDeque::binary_search_by_key
    /// [`partition_point`]: VVecDeque::partition_point
    ///
    /// # Examples
    ///
    ...
    /// Binary searches this `VVecDeque` with a comparator function.
    ///
    /// The comparator function should return an order code that indicates
    /// whether its argument is `Less`, `Equal` or `Greater` the desired
    /// target.
    /// If the `VVecDeque` is not sorted or if the comparator function does not
    /// implement an order consistent with the sort order of the underlying
    /// `VVecDeque`, the returned result is unspecified and meaningless.
    ///
    /// If the value is found then [`Result::Ok`] is returned, containing the
    /// index of the matching element. If there are multiple matches, then any
    /// one of the matches could be returned. If the value is not found then
    ...
    /// Binary searches this `VVecDeque` with a key extraction function.
    ///
    ...
    ///
    /// [`make_contiguous().sort_by_key()`]: VVecDeque::make_contiguous
    /// [`binary_search`]: VVecDeque::binary_search
    /// [`binary_search_by`]: VVecDeque::binary_search_by
    /// [`partition_point`]: VVecDeque::partition_point
    ///
...
impl<T: Clone, A: Allocator> VVecDeque<T, A> {
    /// Modifies the deque in-place so that `len()` is equal to new_len,
...
impl<T: Clone, A: Allocator> SpecExtendFromWithin for VVecDeque<T, A> {
...
impl<T: TrivialClone, A: Allocator> SpecExtendFromWithin for VVecDeque<T, A> {
    unsafe fn spec_extend_from_within(&mut self, src: Range<usize>) {
        let dst = self.len();
        let count = src.end - src.start;
        let src = src.start;
...
impl<T: PartialEq, A: Allocator> PartialEq for VVecDeque<T, A> {
...
impl<T: Eq, A: Allocator> Eq for VVecDeque<T, A> {}

__impl_slice_eq1! { [] VVecDeque<T, A>, Vec<U, A>, }
__impl_slice_eq1! { [] VVecDeque<T, A>, &[U], }
__impl_slice_eq1! { [] VVecDeque<T, A>, &mut [U], }
__impl_slice_eq1! { [const N: usize] VVecDeque<T, A>, [U; N], }
__impl_slice_eq1! { [const N: usize] VVecDeque<T, A>, &[U; N], }
__impl_slice_eq1! { [const N: usize] VVecDeque<T, A>, &mut [U; N], }

impl<T: PartialOrd, A: Allocator> PartialOrd for VVecDeque<T, A> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        self.iter().partial_cmp(other.iter())
    }
}

impl<T: Ord, A: Allocator> Ord for VVecDeque<T, A> {
    #[inline]
...
impl<T: Hash, A: Allocator> Hash for VVecDeque<T, A> {
...
impl<T, A: Allocator> Index<usize> for VVecDeque<T, A> {
    type Output = T;
...
impl<T, A: Allocator> IndexMut<usize> for VVecDeque<T, A> {
    #[inline]
...
impl<T> FromIterator<T> for VVecDeque<T> {
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> VVecDeque<T> {
        SpecFromIter::spec_from_iter(iter.into_iter())
    }
}

impl<T, A: Allocator> IntoIterator for VVecDeque<T, A> {
    type Item = T;
    type IntoIter = VVecDequeIntoIter<T, A>;

    /// Consumes the deque into a front-to-back iterator yielding elements by
    /// value.
    fn into_iter(self) -> VVecDequeIntoIter<T, A> {
        VVecDequeIntoIter::new(self)
    }
}

impl<'a, T, A: Allocator> IntoIterator for &'a VVecDeque<T, A> {
    type Item = &'a T;
    type IntoIter = VVecDequeIter<'a, T>;

    fn into_iter(self) -> VVecDequeIter<'a, T> {
        self.iter()
    }
}

impl<'a, T, A: Allocator> IntoIterator for &'a mut VVecDeque<T, A> {
    type Item = &'a mut T;
    type IntoIter = VVecDequeIterMut<'a, T>;

    fn into_iter(self) -> VVecDequeIterMut<'a, T> {
        self.iter_mut()
    }
}

impl<T, A: Allocator> Extend<T> for VVecDeque<T, A> {
    fn extend<I: IntoIterator<Item = T>>(&mut self, iter: I) {
        // lazy_loss_recovery: finish a forgotten drain before extending (spec_extend reads len directly).
        // The actual extend runs in a separate, never-inlined function so the (never-taken) guard
        // call here cannot merge into and poison the specialized bulk-copy path (e.g. the u8 memcpy).
        self.restore_wf_wo_data_loss();
        #[inline(never)]
        fn extend_after_wf<T, A: Allocator, I: Iterator<Item = T>>(this: &mut VVecDeque<T, A>, iter: I) {
            <VVecDeque<T, A> as SpecExtend<T, I>>::spec_extend(this, iter);
        }
...
impl<'a, T: 'a + Copy, A: Allocator> Extend<&'a T> for VVecDeque<T, A> {
...
impl<T: fmt::Debug, A: Allocator> fmt::Debug for VVecDeque<T, A> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_list().entries(self.iter()).finish()
    }
}

impl<T, A: Allocator> From<Vec<T, A>> for VVecDeque<T, A> {
    /// Turn a [`Vec<T>`] into a [`VVecDeque<T>`].
    ///
    /// [`Vec<T>`]: Vec
    /// [`VVecDeque<T>`]: std::collections::VVecDeque
    ///
    ...
    fn from(other: Vec<T, A>) -> Self {
        ...
        Self {
            head: 0,
            len,
            buf: unsafe { VRawVec::from_raw_parts_in(ptr, cap, alloc) },
            pending: None,
        }
    }
}

impl<T, A: Allocator> From<VVecDeque<T, A>> for Vec<T, A> {
    /// Turn a [`VVecDeque<T>`] into a [`Vec<T>`].
    ///
    /// [`Vec<T>`]: Vec
    /// [`VVecDeque<T>`]: std::collections::VVecDeque
    ///
    ...
    /// # Examples
    ///
    fn from(mut other: VVecDeque<T, A>) -> Self {
...
impl<T, const N: usize> From<[T; N]> for VVecDeque<T> {
    /// Converts a `[T; N]` into a `VVecDeque<T>`.
    ///
    fn from(arr: [T; N]) -> Self {
        let mut deq = VVecDeque::with_capacity(N);
        let arr = ManuallyDrop::new(arr);
        if !<T>::IS_ZST {
            // SAFETY: VVecDeque::with_capacity ensures that there is enough capacity.
            unsafe {
