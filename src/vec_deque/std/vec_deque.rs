...
impl<T> VVecDeque<T> {
    ...
    pub const fn new() -> VVecDeque<T> {
        ...
        VVecDeque { head: 0, len: 0, buf: VRawVec::new() }
    }
    ...
    pub fn try_with_capacity(capacity: usize) -> Result<VVecDeque<T>, TryReserveError> {
        Ok(VVecDeque { head: 0, len: 0, buf: VRawVec::try_with_capacity_in(capacity, Global)? })
    }
}
...
impl<T, A: Allocator> VVecDeque<T, A> {
    ...
    pub const fn new_in(alloc: A) -> VVecDeque<T, A> {
        VVecDeque { head: 0, len: 0, buf: VRawVec::new_in(alloc) }
    }
    ...
    pub fn with_capacity_in(capacity: usize, alloc: A) -> VVecDeque<T, A> {
        VVecDeque { head: 0, len: 0, buf: VRawVec::with_capacity_in(capacity, alloc) }
    }
        ...
        R: RangeBounds<usize>,
    {
        ...
        // drain_len and orig_len respectively on the VVecDequeDrain. This also
        // truncates the effective array such that if the VVecDequeDrain is leaked, we
        ...
        // the drain is complete and the VVecDequeDrain destructor is run.

        unsafe { VVecDequeDrain::new(self, drain_start, drain_len) }
    }
    ...
    ///
    /// It is unspecified how many elements are removed from the deque if the `VVecDequeSplice` value is
    /// leaked.
    ///
    /// The input iterator `replace_with` is only consumed when the `VVecDequeSplice` value is dropped.
    ///
    /// This is optimal if:
    ///
    ...
    {
        VVecDequeSplice { drain: self.drain(range), replace_with: replace_with.into_iter() }
    }
...
impl<T, A: Allocator> From<Vec<T, A>> for VVecDeque<T, A> {
    ...
    fn from(other: Vec<T, A>) -> Self {
        ...
        Self { head: 0, len, buf: unsafe { VRawVec::from_raw_parts_in(ptr, cap, alloc) } }
    }
}
