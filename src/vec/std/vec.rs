...
impl<T, A: Allocator> VVec<T, A> {
    ...
    pub fn with_capacity_in(capacity: usize, alloc: A) -> Self {
        VVec { buf: VRawVec::with_capacity_in(capacity, alloc), len: 0 }
    }
...
impl<T, A: Allocator> VVec<T, A> {
    ...
    pub const fn new_in(alloc: A) -> Self {
        VVec { buf: VRawVec::new_in(alloc), len: 0 }
    }
    ...
    pub fn try_with_capacity_in(capacity: usize, alloc: A) -> Result<Self, TryReserveError> {
        Ok(VVec { buf: VRawVec::try_with_capacity_in(capacity, alloc)?, len: 0 })
    }
    ...
    pub unsafe fn from_raw_parts_in(
        ...
        unsafe { VVec { buf: VRawVec::from_raw_parts_in(ptr, capacity, alloc), len: length } }
    }
    ...
    pub unsafe fn from_parts_in(
        ...
        unsafe { VVec { buf: VRawVec::from_nonnull_in(ptr, capacity, alloc), len: length } }
    }
...
impl<T, A: Allocator> VVec<T, A> {
    ...
    /// if the `VVecSplice` value is leaked.
    ///
    /// The input iterator `replace_with` is only consumed when the `VVecSplice` value is dropped.
    ///
    /// This is optimal if:
    ///
