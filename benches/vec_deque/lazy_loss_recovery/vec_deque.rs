...
//! Benchmarks for `VVecDeque`, a plain copy of the rust-libs 1.96.0 vec_deque benchmarks
//! (alloctests/benches/vec_deque.rs) with the type names V-renamed (`VecDeque` ->
//! `VVecDeque`, `vec_deque::IntoIter` -> `vec_deque::VVecDequeIntoIter`). `feature(test)` is
//! unlocked via RUSTC_BOOTSTRAP=1 (.cargo/config.toml).

#![feature(test)]
#![feature(iter_next_chunk)] // VVecDequeIntoIter::next_chunk exercised by into_iter benches
...
use rustseal::vec_deque::lazy_loss_recovery as vec_deque;
use vec_deque::VVecDeque;


#[bench]
fn bench_new(b: &mut Bencher) {
    b.iter(|| {
        let ring: VVecDeque<i32> = VVecDeque::new();
        black_box(ring);
    })
}
...
fn bench_iter_1000(b: &mut Bencher) {
    let ring: VVecDeque<_> = (0..1000).collect();
...
fn bench_mut_iter_1000(b: &mut Bencher) {
    let mut ring: VVecDeque<_> = (0..1000).collect();
...
fn into_iter_helper<
    T: Copy,
    F: FnOnce(&mut VVecDeque<T>),
    G: FnOnce(&mut vec_deque::VVecDequeIntoIter<T>),
>(
    v: &mut Vec<T>,
    setup: F,
    g: G,
    ...
    let mut deque = VVecDeque::from(mem::take(v));
    setup(&mut deque);
...
fn bench_into_iter_fold(b: &mut Bencher) {
    ...
    // for every iteration.
    b.iter(|| {
        let deque: VVecDeque<usize> = (0..len).collect();
        assert_eq!(deque.len(), deque.capacity());
        let sum = deque.into_iter().fold(0, |a, b| a + b);
        black_box(sum);

        // rotating a full deque doesn't move any memory.
        let mut deque: VVecDeque<usize> = (0..len).collect();
        assert_eq!(deque.len(), deque.capacity());
...
fn bench_from_array_1000(b: &mut Bencher) {
    ...
    b.iter(|| {
        let deq: VVecDeque<_> = array.into();
        black_box(deq);
    })
}
...
fn bench_extend_bytes(b: &mut Bencher) {
    let mut ring: VVecDeque<u8> = VVecDeque::with_capacity(1000);
...
fn bench_extend_trustedlen(b: &mut Bencher) {
    let mut ring: VVecDeque<u16> = VVecDeque::with_capacity(1000);
