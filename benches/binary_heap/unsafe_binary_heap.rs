...
//! Benchmarks for `UnsafeBinaryHeap`, a plain copy of the rust-libs 1.96.0 binary_heap
...
fn bench_find_smallest_1000(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        let mut heap: UnsafeBinaryHeap<_> = iter.by_ref().take(1000).collect();
...
fn bench_peek_mut_deref_mut(b: &mut Bencher) {
    let mut bheap = UnsafeBinaryHeap::from(vec![42]);
...
fn bench_from_vec(b: &mut Bencher) {
    ...
    b.iter(|| UnsafeBinaryHeap::from(vec.clone()))
}

#[bench]
fn bench_into_sorted_vec(b: &mut Bencher) {
    let bheap: UnsafeBinaryHeap<i32> = (0..10_000).collect();
...
fn bench_push(b: &mut Bencher) {
    let mut bheap = UnsafeBinaryHeap::with_capacity(50_000);
    let mut rng = bench_rng();
...
fn bench_pop(b: &mut Bencher) {
    let mut bheap = UnsafeBinaryHeap::with_capacity(10_000);
