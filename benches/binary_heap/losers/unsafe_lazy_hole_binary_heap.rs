...
//! Benchmarks for `UnsafeLazyHoleBinaryHeap`, a plain copy of the rust-libs 1.96.0 binary_heap
...
use rustseal::binary_heap::unsafe_lazy_hole_binary_heap::UnsafeLazyHoleBinaryHeap;
...
fn bench_find_smallest_1000(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        let mut heap: UnsafeLazyHoleBinaryHeap<_> = iter.by_ref().take(1000).collect();
...
fn bench_peek_mut_deref_mut(b: &mut Bencher) {
    let mut bheap = UnsafeLazyHoleBinaryHeap::from(vec![42]);
...
fn bench_from_vec(b: &mut Bencher) {
    ...
    b.iter(|| UnsafeLazyHoleBinaryHeap::from(vec.clone()))
}

#[bench]
fn bench_into_sorted_vec(b: &mut Bencher) {
    let bheap: UnsafeLazyHoleBinaryHeap<i32> = (0..10_000).collect();
...
fn bench_push(b: &mut Bencher) {
    let mut bheap = UnsafeLazyHoleBinaryHeap::with_capacity(50_000);
    let mut rng = bench_rng();
...
fn bench_pop(b: &mut Bencher) {
    let mut bheap = UnsafeLazyHoleBinaryHeap::with_capacity(10_000);
