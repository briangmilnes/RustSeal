...
//! Tests for `SafeBinaryHeap`, a plain copy of the rust-libs 1.96.0 binary_heap test
...
use rustseal::binary_heap::safe_binary_heap::{SafeBinaryHeap, SafeDrain, SafePeekMut};
...
fn test_iterator() {
    let data = vec![5, 9, 3];
    let iterout = [9, 5, 3];
    let heap = SafeBinaryHeap::from(data);
    let mut i = 0;
    for el in &heap {
...
fn test_iter_rev_cloned_collect() {
    ...
    let pq = SafeBinaryHeap::from(data);
...
fn test_into_iter_sorted_collect() {
    let heap = SafeBinaryHeap::from(vec![2, 4, 6, 2, 1, 8, 10, 3, 5, 7, 0, 9, 1]);
...
fn test_drain_sorted_collect() {
    let mut heap = SafeBinaryHeap::from(vec![2, 4, 6, 2, 1, 8, 10, 3, 5, 7, 0, 9, 1]);
...
fn test_peek_and_pop() {
    ...
    sorted.sort();
    let mut heap = SafeBinaryHeap::from(data);
    while !heap.is_empty() {
...
fn test_peek_mut_pop() {
    ...
    assert_eq!(heap.peek(), Some(&10));
    {
        let mut top = heap.peek_mut().unwrap();
        *top -= 2;
        assert_eq!(SafePeekMut::pop(top), 8);
    }
    assert_eq!(heap.peek(), Some(&9));
}
...
#[test]
fn test_push() {
    let mut heap = SafeBinaryHeap::from(vec![2, 4, 9]);
...
#[test]
fn test_push_unique() {
    let mut heap = SafeBinaryHeap::<Box<_>>::from(vec![Box::new(2), Box::new(4), Box::new(9)]);
    assert_eq!(heap.len(), 3);
    assert!(**heap.peek().unwrap() == 9);
...
fn check_to_vec(mut data: Vec<i32>) {
    let heap = SafeBinaryHeap::from(data.clone());
...
fn test_in_place_iterator_specialization() {
    ...
    let heap: SafeBinaryHeap<_> = src.into_iter().map(std::convert::identity).collect();
...
#[test]
fn test_empty_pop() {
    let mut heap = SafeBinaryHeap::<i32>::new();
    assert!(heap.pop().is_none());
}

#[test]
fn test_empty_peek() {
    let empty = SafeBinaryHeap::<i32>::new();
    assert!(empty.peek().is_none());
}

#[test]
fn test_empty_peek_mut() {
    let mut empty = SafeBinaryHeap::<i32>::new();
...
fn test_from_iter() {
    ...
    let mut q: SafeBinaryHeap<_> = xs.iter().rev().cloned().collect();
...
#[test]
fn test_drain() {
    let mut q: SafeBinaryHeap<_> = [9, 8, 7, 6, 5, 4, 3, 2, 1].iter().cloned().collect();
...
fn test_drain_sorted_leak() {
    ...
    let mut q = SafeBinaryHeap::from(vec![
...
fn test_drain_forget() {
    ...
    let mut q =
        SafeBinaryHeap::from(vec![a.spawn(Panic::Never), b.spawn(Panic::Never), c.spawn(Panic::Never)]);
...
#[test]
fn test_append() {
    let mut a = SafeBinaryHeap::from(vec![-10, 1, 2, 3, 3]);
    let mut b = SafeBinaryHeap::from(vec![-20, 5, 43]);
...
fn test_extend_specialization() {
    let mut a = SafeBinaryHeap::from(vec![-10, 1, 2, 3, 3]);
    let b = SafeBinaryHeap::from(vec![-20, 5, 43]);
...
fn assert_covariance() {
    fn drain<'new>(d: SafeDrain<'static, &'static str>) -> SafeDrain<'new, &'new str> {
        d
    }
}
...
#[test]
fn test_retain() {
    let mut a = SafeBinaryHeap::from(vec![100, 10, 50, 1, 2, 20, 30]);
    a.retain(|&x| x != 2);
...
fn test_retain_catch_unwind() {
    let mut heap = SafeBinaryHeap::from(vec![3, 1, 2]);
...
fn panic_safe() {
    ...
    for _ in 0..ntest {
        for i in 1..=DATASZ {
            ...
            let mut heap = SafeBinaryHeap::from(panic_ords);
            let inner_data;
