...
//! Tests for `UnsafeNopanicBinaryHeap`, a plain copy of the rust-libs 1.96.0 binary_heap test
...
use rustseal::binary_heap::unsafe_nopanic_binary_heap::{UnsafeNopanicBinaryHeap, UnsafeNopanicDrain, UnsafeNopanicPeekMut};
...
fn test_iterator() {
    let data = vec![5, 9, 3];
    let iterout = [9, 5, 3];
    let heap = UnsafeNopanicBinaryHeap::from(data);
    let mut i = 0;
    for el in &heap {
...
fn test_iter_rev_cloned_collect() {
    ...
    let pq = UnsafeNopanicBinaryHeap::from(data);
...
fn test_into_iter_sorted_collect() {
    let heap = UnsafeNopanicBinaryHeap::from(vec![2, 4, 6, 2, 1, 8, 10, 3, 5, 7, 0, 9, 1]);
...
fn test_drain_sorted_collect() {
    let mut heap = UnsafeNopanicBinaryHeap::from(vec![2, 4, 6, 2, 1, 8, 10, 3, 5, 7, 0, 9, 1]);
...
fn test_peek_and_pop() {
    ...
    sorted.sort();
    let mut heap = UnsafeNopanicBinaryHeap::from(data);
    while !heap.is_empty() {
...
// UnsafeNopanicBinaryHeap INTENTIONALLY drops the forget guarantee its O(log n) peek_mut buys
// speed by not holding the rest of the heap aside, so forgetting a mutated guard skips the
// Drop sift and leaves the heap in broken order. This test asserts the OPPOSITE invariant
// (forget leaves a valid/sorted heap), which holds for safe_binary_heap and the unsafe heap
// but NOT here — so it would fail by design. #[ignore]d, not deleted, to document the trade.
#[test]
#[ignore = "safe_opt drops the forget guarantee for an O(log n) peek_mut; forgetting a mutated guard leaves an unsorted heap by design"]
...
fn test_peek_mut_pop() {
    ...
    assert_eq!(heap.peek(), Some(&10));
    {
        let mut top = heap.peek_mut().unwrap();
        *top -= 2;
        assert_eq!(UnsafeNopanicPeekMut::pop(top), 8);
    }
    assert_eq!(heap.peek(), Some(&9));
}
...
#[test]
fn test_push() {
    let mut heap = UnsafeNopanicBinaryHeap::from(vec![2, 4, 9]);
...
#[test]
fn test_push_unique() {
    let mut heap = UnsafeNopanicBinaryHeap::<Box<_>>::from(vec![Box::new(2), Box::new(4), Box::new(9)]);
    assert_eq!(heap.len(), 3);
    assert!(**heap.peek().unwrap() == 9);
...
fn check_to_vec(mut data: Vec<i32>) {
    let heap = UnsafeNopanicBinaryHeap::from(data.clone());
...
fn test_in_place_iterator_specialization() {
    ...
    let heap: UnsafeNopanicBinaryHeap<_> = src.into_iter().map(std::convert::identity).collect();
...
#[test]
fn test_empty_pop() {
    let mut heap = UnsafeNopanicBinaryHeap::<i32>::new();
    assert!(heap.pop().is_none());
}

#[test]
fn test_empty_peek() {
    let empty = UnsafeNopanicBinaryHeap::<i32>::new();
    assert!(empty.peek().is_none());
}

#[test]
fn test_empty_peek_mut() {
    let mut empty = UnsafeNopanicBinaryHeap::<i32>::new();
...
fn test_from_iter() {
    ...
    let mut q: UnsafeNopanicBinaryHeap<_> = xs.iter().rev().cloned().collect();
...
#[test]
fn test_drain() {
    let mut q: UnsafeNopanicBinaryHeap<_> = [9, 8, 7, 6, 5, 4, 3, 2, 1].iter().cloned().collect();
...
fn test_drain_sorted_leak() {
    ...
    let mut q = UnsafeNopanicBinaryHeap::from(vec![
...
fn test_drain_forget() {
    ...
    let mut q =
        UnsafeNopanicBinaryHeap::from(vec![a.spawn(Panic::Never), b.spawn(Panic::Never), c.spawn(Panic::Never)]);
...
#[test]
fn test_extend_ref() {
    let mut a = UnsafeNopanicBinaryHeap::new();
    a.push(1);
    a.push(2);
    ...
    a.push(1);
    a.push(2);
    let mut b = UnsafeNopanicBinaryHeap::new();
    b.push(3);
    b.push(4);
    b.push(5);
...
#[test]
fn test_append() {
    let mut a = UnsafeNopanicBinaryHeap::from(vec![-10, 1, 2, 3, 3]);
    let mut b = UnsafeNopanicBinaryHeap::from(vec![-20, 5, 43]);
...
fn test_extend_specialization() {
    let mut a = UnsafeNopanicBinaryHeap::from(vec![-10, 1, 2, 3, 3]);
    let b = UnsafeNopanicBinaryHeap::from(vec![-20, 5, 43]);
...
fn assert_covariance() {
    fn drain<'new>(d: UnsafeNopanicDrain<'static, &'static str>) -> UnsafeNopanicDrain<'new, &'new str> {
        d
    }
}
...
#[test]
fn test_retain() {
    let mut a = UnsafeNopanicBinaryHeap::from(vec![100, 10, 50, 1, 2, 20, 30]);
    a.retain(|&x| x != 2);
...
fn test_retain_catch_unwind() {
    let mut heap = UnsafeNopanicBinaryHeap::from(vec![3, 1, 2]);
...
#[test]
#[cfg_attr(not(panic = "unwind"), ignore = "test requires unwinding support")]
// unsafe_nopanic OMITS the Hole's Drop guard by design: a `T: Ord` panic mid-sift leaves a
// duplicated slot and drops the carried value -> double-drop on unwind. This test exercises
// exactly that (PanicOrd panics in the comparison during push->sift_up), so it cannot hold
// here. #[ignore]d, not deleted, to document that this variant trades panic safety for the
// bare hole technique.
#[ignore = "unsafe_nopanic has no panic guard; a comparison panic mid-sift double-drops by design"]
fn panic_safe() {
    use std::cmp;
    ...
    for _ in 0..ntest {
        for i in 1..=DATASZ {
            ...
            let mut heap = UnsafeNopanicBinaryHeap::from(panic_ords);
            let inner_data;
