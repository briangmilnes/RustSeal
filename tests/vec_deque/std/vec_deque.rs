...
use rustseal::vec_deque::std::VVecDeque;
use rustseal::vec_deque::std::VVecDequeDrain;
...
// --- Forget / no-iterator-drop data-loss tests (added to elicit the leak-amplification
// behavior that the std docs only hand-wave as "leaked elements arbitrarily"). These PASS
// because they assert the current, defective behavior: forgetting the iterator skips its
// Drop, so the eagerly-shortened `len` is never restored and elements are leaked (their
// own Drop never runs). The names state the data loss each one demonstrates.

#[test]
fn test_drain_forget_from_zero_leaks_whole_deque() {
    // A drain starting at 0 sets the deque's len to 0 at construction. Forgetting the
    // iterator leaves it at 0 forever -> the ENTIRE deque is leaked (no Drop runs).
    ...
    assert_eq!(v.len(), 0); // len parked at drain_start == 0, never restored
    assert_eq!(DROPS.get(), 0); // nothing dropped yet
    drop(v);
    assert_eq!(DROPS.get(), 0); // all 5 elements leaked: their Drop never ran
}

#[test]
fn test_drain_forget_leaks_drained_range_and_tail() {
    // A drain starting at index 1 sets len to 1 (just the head). Forgetting the iterator
    // leaks the drained range AND the tail beyond it -- only the head survives.
    struct_with_counted_drop!(D(u32), DROPS);
    ...
    std::mem::forget(v.drain(1..3)); // drain_start == 1 -> len parked at 1

    assert_eq!(v.len(), 1); // only the head (D(0)) is on the books
    assert_eq!(DROPS.get(), 0);
    drop(v);
    // Only D(0) is dropped; the drained range (1, 2) and the tail (3, 4) are leaked.
    assert_eq!(DROPS.get(), 1);
}

#[test]
fn test_extract_if_forget_leaks_whole_deque() {
    // extract_if sets the deque's len to 0 at construction (its leak-amplification guard).
    // Forgetting the iterator leaves it at 0 -> the ENTIRE deque is leaked.
    struct_with_counted_drop!(D(u32), DROPS);
    ...
    assert_eq!(v.len(), 0); // len parked at 0, never restored
    assert_eq!(DROPS.get(), 0);
    drop(v);
    assert_eq!(DROPS.get(), 0); // all 5 elements leaked
}
...
fn test_splice_forget_leaks_range_tail_and_replacement() {
    // splice holds a Drain, so it inherits drain's leak amplification: forgetting the Splice parks
    // the deque's len at the splice start and never restores it. This PASSES by asserting the data
    // loss the std docs only hand-wave ("unspecified how many elements are removed ... if leaked").
    // The drained range, the entire tail, AND the un-inserted replacement are all leaked.
    struct_with_counted_drop!(D(u32), DROPS);
    ...
    let replacement = [D(100), D(101)]; // counted too; never inserted on forget

    std::mem::forget(v.splice(2..4, replacement));

    assert_eq!(v.len(), 2); // parked at the splice start (2); only [0, 1] remain on the books
    assert_eq!(DROPS.get(), 0);
    drop(v);
    // Only D(0), D(1) are dropped. LEAKED: the range D(2),D(3) + the tail D(4),D(5)
    // + the replacement D(100),D(101) -- 6 elements whose Drop never runs.
    assert_eq!(DROPS.get(), 2);
}
