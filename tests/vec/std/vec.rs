...
use rustseal::vec::std::{VVec, VVecDrain, VVecIntoIter, VVecPeekMut};

/// Local `vec!` building a `VVec` (the prelude `vec!` would build a std `Vec`). The
/// `[elem; n]` form uses the extracted free fn `rustseal::vec::std::from_elem`.
macro_rules! vec {
    () => { VVec::new() };
    ($elem:expr; $n:expr) => { rustseal::vec::std::from_elem($elem, $n) };
    ($($x:expr),+ $(,)?) => {{
        let mut v = VVec::new();
        $( v.push($x); )+
        v
...
// --- Forget / no-iterator-drop data-loss tests. std parks `len` short at drain construction and
// relies solely on `VVecDrain::drop` to restore it; a `mem::forget` skips that drop, so elements
// leak (their own Drop never runs). These PASS by asserting that current, defective behavior.

#[test]
fn test_drain_forget_from_zero_leaks_whole_vec() {
    ...
    assert_eq!(v.len(), 0); // parked at 0, never restored
    assert_eq!(DROPS.get(), 0);
    drop(v);
    assert_eq!(DROPS.get(), 0); // all 5 leaked: their Drop never ran
}

#[test]
fn test_drain_forget_leaks_range_and_tail() {
    struct_with_counted_drop!(D(u32), DROPS);
    let mut v = VVec::new();
    for i in 0..5 {
        v.push(D(i)); // [0, 1, 2, 3, 4]
    }
    std::mem::forget(v.drain(1..3)); // parked at 1 (just the head)
    assert_eq!(v.len(), 1);
    assert_eq!(DROPS.get(), 0);
    drop(v);
    // Only D(0) drops; the range (1, 2) and the tail (3, 4) are leaked.
    assert_eq!(DROPS.get(), 1);
}

#[test]
fn test_extract_if_forget_leaks_whole_vec() {
    // extract_if parks `len` at 0 at construction (leak-amplification guard); forget -> whole vec leaked.
    struct_with_counted_drop!(D(u32), DROPS);
    let mut v = VVec::new();
    for i in 0..5 {
        v.push(D(i));
    ...
    assert_eq!(DROPS.get(), 0);
    drop(v);
    assert_eq!(DROPS.get(), 0); // all 5 leaked
}
...
#[test]
fn test_splice_forget_leaks_range_tail_and_replacement() {
    // splice holds a VVecDrain, inheriting its leak amplification: forget parks len at the splice
    // start and never restores it. Leaks the range + tail + the un-inserted replacement.
    struct_with_counted_drop!(D(u32), DROPS);
    let mut v = VVec::new();
    for i in 0..6 {
        v.push(D(i)); // [0..6]
    ...
    assert_eq!(v.len(), 2); // parked at 2; only [0,1] on the books
    drop(v);
    // Only D(0), D(1) drop; range {2,3} + tail {4,5} + replacement {100,101} (6) leaked.
    assert_eq!(DROPS.get(), 2);
}
